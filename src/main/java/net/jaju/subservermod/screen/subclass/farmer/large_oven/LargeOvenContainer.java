...
public class LargeOvenContainer extends AbstractContainerMenu {
    private final LargeOvenBlockEntity blockEntity;
    private final ContainerLevelAccess access;

    public LargeOvenContainer(int id, Inventory playerInventory, BlockPos pos) {
        super(ModContainers.LARGE_OVEN_MENU.get(), id);
        this.blockEntity = (LargeOvenBlockEntity) playerInventory.player.level().getBlockEntity(pos);
    ...
    private void addOvenSlots() {
        ...
        for (int row = 0; row < 4; row++) {
            for (int col = 0; col < 4; col++) {
                this.addSlot(new LimitedSlot(itemHandler, col + row * 4, 64 + col * 20,  3 + row * 20, 24));
            }
        }
        this.addSlot(new LimitedSlot(itemHandler, 16, 84, 75, 0));
    }
    ...
    public boolean stillValid(Player player) {
        return stillValid(access, player, ModBlocks.LARGE_OVEN_BLOCK.get());
    }
    ...
    public LargeOvenBlockEntity getBlockEntity() {
        return blockEntity;
    }
}