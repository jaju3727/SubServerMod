...
public class OvenContainer extends AbstractContainerMenu {
    private final OvenBlockEntity blockEntity;
    private final ContainerLevelAccess access;

    public OvenContainer(int id, Inventory playerInventory, BlockPos pos) {
        super(ModContainers.OVEN_MENU.get(), id);
        this.blockEntity = (OvenBlockEntity) playerInventory.player.level().getBlockEntity(pos);
    ...
    public boolean stillValid(Player player) {
        return stillValid(access, player, ModBlocks.OVEN_BLOCK.get());
    }
    ...
    public ItemStack quickMoveStack(Player player, int index) {
        ...
        if (slot != null && slot.hasItem()) {
            ...
            // If the slot is within the oven inventory, move to player inventory
            if (index < 10) {
                if (!this.moveItemStackTo(stackInSlot, 10, this.slots.size(), true)) {
                    return ItemStack.EMPTY;
                }
            } else { // If the slot is within the player inventory, move to oven inventory
